...
def enable_simple_preset_wheel_scrolling(combo_box: QComboBox, preset_order_manager):
    """
    为预设QComboBox启用简化的滚轮滚动功能，只在男主和女主之间切换
    
    Args:
        combo_box: 要启用滚轮功能的QComboBox
        preset_order_manager: 预设排序管理器，用于获取男主女主预设
    """
    def custom_wheel_event(event):
        """自定义滚轮事件处理，只在男主和女主之间切换"""
        if combo_box.count() == 0:
            event.ignore()
            return
            
        if not preset_order_manager:
            event.ignore()
            return
            
        # 获取所有男主和女主预设
        male_leads = list(preset_order_manager.male_leads) if preset_order_manager.male_leads else []
        female_leads = list(preset_order_manager.female_leads) if preset_order_manager.female_leads else []
        
        if not male_leads and not female_leads:
            event.ignore()
            return
        ...
        if delta > 0:  # 向上滚动
            ...
            if male_leads:
                # 如果当前不是男主，切换到第一个男主
                if current_text not in male_leads:
                    male_lead = male_leads[0]
                    index = combo_box.findText(male_lead)
                    if index >= 0:
            ...
            if female_leads:
                # 如果当前不是女主，切换到第一个女主
                if current_text not in female_leads:
                    female_lead = female_leads[0]
                    index = combo_box.findText(female_lead)
                    if index >= 0:
                        combo_box.setCurrentIndex(index)
    ...
    # 替换wheelEvent方法
    combo_box.wheelEvent = custom_wheel_event

def disable_combo_wheel_scrolling(combo_box: QComboBox):
    """
    禁用QComboBox的滚轮滚动功能
    
    Args:
        combo_box: 要禁用滚轮功能的QComboBox
    """
    ...
    # 替换wheelEvent方法为忽略事件
    combo_box.wheelEvent = ignore_wheel_event
...
# 请用这个完整、正确缩进的版本，替换您文件中现有的整个 PresetSettingCard 类
...
class PresetSettingCard(Card):
    ...
    def init_ui(self):
        ...
        # --- BUG 2 修复 START ---
        # 行为变更: 将加载默认参数的操作提前到所有控件初始化之前。
        # 原因: 确保所有需要使用默认值的控件（如语速、分段间隔等）在创建时就能获得正确的值，
        # 而不是被硬编码的值覆盖。
        ...
        self.male_lead_check = QCheckBox("主角1")
        ...
        self.female_lead_check = QCheckBox("主角2")
        ...
        role_note = QLabel("(下次启动时生效，主角1排前，主角2排后)")
        ...
        # --- BUG 2 修复 START ---
        # 行为变更: 使用从 default_params 加载的值来初始化语速控件。
        # 原因: 修复了控件值被硬编码为 1.0，导致“参数全部修改”功能对默认语速无效的 bug。
        ...
        # 设置文本切分默认值
        split_method = default_params.get('text_split_method', 'cut1')
        split_index = 1  # 默认为cut1
        for i, option in enumerate(split_options):
            if option.startswith(split_method):
                split_index = i
                break
        ...
        self.interval_spin.setValue(default_params.get('fragment_interval', 0.3))
        ...
        self.temp_spin.setValue(default_params.get('temperature', 0.6))  # 🔧 更新默认值为0.6
        ...
        self.topk_spin.setValue(default_params.get('top_k', 15))  # v4版本推荐值
        ...
        self.topp_spin.setValue(default_params.get('top_p', 1.0))  # v4版本推荐值
        ...
        self.penalty_spin.setValue(default_params.get('repetition_penalty', 1.35))  # v4版本推荐值
        ...
        self.lang_combo.setCurrentText(default_params.get('ref_language', 'all_zh'))  # 默认为all_zh
        ...
        self.batch_size_spin.setValue(default_params.get('batch_size', 1))  # 默认为1
        self.batch_size_spin.setMinimumHeight(control_height)
        
        self.parallel_infer_check = QCheckBox("并行推理")
        self.parallel_infer_check.setChecked(default_params.get('parallel_infer', True))  # 🔧 更新默认值为True
        
        self.split_bucket_check = QCheckBox("分桶处理")
        self.split_bucket_check.setChecked(default_params.get('split_bucket', False))  # 默认禁用
        ...
        self.sample_steps_spin.setValue(default_params.get('sample_steps', 8))  # v4默认值调整为8
        ...
        self.if_sr_check.setChecked(default_params.get('if_sr', True))  # 🔧 更新默认值为True
        ...
        self.aux_ref_check.setChecked(default_params.get('aux_ref_enabled', False))  # 默认关闭
        ...
        # 参数全部修改按钮
        self.update_all_btn = QPushButton("参数全部修改")
        self.update_all_btn.setFixedHeight(40)
        self.update_all_btn.setFixedWidth(140)
        self.update_all_btn.setStyleSheet(f"""
            ...
            border-radius: 4px;
        """)
        self.update_all_btn.setToolTip("将当前节奏设置和推理设置应用到所有已保存的预设，以及修改默认参数")
        self.update_all_btn.clicked.connect(self.update_all_presets_parameters)
    ...
    def save_preset(self):
        ...
        settings = {
            ...
            # --- BUG 1 修复 START ---
            # 行为变更: 同时保存 `speed` 和 `speed_factor` 两个键。
            # 原因: 为了兼容可能在后端（gpt_sovits.py）使用不同键名的情况，确保语速设置能被正确识别。
            'speed_factor': self.speed_factor_spin.value(),  # 语速参数
            'speed': self.speed_factor_spin.value(),         # 增加兼容性键
            # --- BUG 1 修复 END ---
    ...
    def update_all_presets_parameters(self):
        """参数全部修改：将当前节奏设置和推理设置应用到所有已保存的预设，以及修改默认参数"""
        
        # 确认对话框
        reply = QMessageBox.question(self, "确认操作", 
                                   "此操作将会：\n\n"
                                   "1. 将当前的节奏设置和推理设置应用到所有已保存的预设\n"
                                   "2. 修改系统默认参数为当前设置\n\n"
                                   "这将覆盖所有预设的相关参数，是否继续？",
                                   QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                   QMessageBox.StandardButton.No)
        ...
        try:
            # 获取当前的参数设置
            current_params = {
                'text_split_method': self.split_combo.currentText().split('（')[0],
                'fragment_interval': self.interval_spin.value(),
                # --- BUG 1 修复 START ---
                # 行为变更: 同步更新 `speed` 和 `speed_factor` 两个键。
                # 原因: 确保在“参数全部修改”时，这两个兼容性键都被写入，保证功能的一致性。
                'speed_factor': self.speed_factor_spin.value(),
                'speed': self.speed_factor_spin.value(),
                # --- BUG 1 修复 END ---
                'temperature': self.temp_spin.value(),
                'top_k': int(self.topk_spin.value()),
                'top_p': self.topp_spin.value(),
                'repetition_penalty': self.penalty_spin.value(),
                'ref_language': self.lang_combo.currentText(),
                'text_lang': self.lang_combo.currentText(),
                'text_language': self.lang_combo.currentText(),
                'batch_size': self.batch_size_spin.value(),
                'parallel_infer': self.parallel_infer_check.isChecked(),
                'split_bucket': self.split_bucket_check.isChecked(),
                'sample_steps': self.sample_steps_spin.value(),
                'if_sr': self.if_sr_check.isChecked(),
                'aux_ref_enabled': self.aux_ref_check.isChecked(),
            }
            ...
            for preset_name, settings, audio_files in all_presets:
                # 更新预设的参数，保留基本信息（模型路径、音频文件等）
                settings.update(current_params)
                
                # 重新保存预设
                if self.preset_manager.add_preset(preset_name, settings, audio_files):
                    updated_count += 1
                else:
                    print(f"更新预设 {preset_name} 失败")
            
            # 保存当前参数为默认参数（写入配置文件）
            self._save_default_parameters(current_params)
            
            # 显示成功消息
            QMessageBox.information(self, "操作完成", 
                                   f"成功更新了 {updated_count} 个预设的参数\n"
                                   f"并已将当前参数设置为系统默认值")
            ...
            QMessageBox.critical(self, "错误", f"参数全部修改失败：{str(e)}")
    
    def _save_default_parameters(self, params: dict):
        """保存默认参数到配置文件"""
        import json
        import os
        
        config_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "default_parameters.json")
        
        try:
            with open(config_file, 'w', encoding='utf-8') as f:
                json.dump(params, f, ensure_ascii=False, indent=2)
            print(f"默认参数已保存到: {config_file}")
        except Exception as e:
            print(f"保存默认参数失败: {e}")
    ...
    def _load_default_parameters(self):
        ...
        try:
            if os.path.exists(config_file):
                ...
                print(f"已加载自定义默认参数: {config_file}")
        except Exception as e:
            print(f"加载默认参数失败，使用内置默认值: {e}")
...
class FirstSegmentCard(Card):
    def __init__(self, segments: List[str], preset_manager: PresetManager, preset_order_manager: PresetOrderManager = None, parent=None):
        ...
        self.current_preset_settings = None
        self.processed_segments = self.segments.copy()
        ...
        self.volume_check_timer.start(1000)
    ...
    def global_replace(self, search_text: str, replace_text: str):
        ...
        for i in range(len(self.processed_segments)):
            original_text = self.processed_segments[i]
            if search_text in original_text:
                count_before = original_text.count(search_text)
                self.processed_segments[i] = original_text.replace(search_text, replace_text)
                replace_count += count_before
        
        replace_history_manager.add_replace_record(search_text, replace_text)
        self.update_text_display()
        QMessageBox.information(self, "成功", f"已全局替换 {search_text} 为 {replace_text}，共替换了 {replace_count} 處文本")
    ...
    def remove_selected_history(self, combo: QComboBox):
        ...
        search_text, _ = combo.currentData()
        
        reply = QMessageBox.question(
            self, 
            "确认删除", 
            f"确定要删除替换规则 '{combo.currentText()}' 吗？",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, 
            QMessageBox.StandardButton.No
        )
    ...
    def apply_all_history(self):
        """应用所有历史替换 (单人模式版)"""
        for i in range(len(self.processed_segments)):
            self.processed_segments[i] = replace_history_manager.apply_all_replacements(self.processed_segments[i])
    ...
    def init_ui(self):
        ...
        self.next_btn.setEnabled(len(self.segments) > 1)
        ...
        replace_frame.setStyleSheet("""
            QFrame {
                border: 1px solid #d4c5b9; border-radius: 8px; background-color: #ffffff;
        ...
        replace_header.setStyleSheet("font-size: 14px; font-weight: bold; color: #C67A8A;")
        ...
        replace_button.setStyleSheet("background-color: #C67A8A; color: white; border: none;")
        ...
        disable_combo_wheel_scrolling(history_combo)
        ...
        self.find_edit.setPlaceholderText("在当前段落查找")
        self.replace_edit = QLineEdit()
        self.replace_edit.setPlaceholderText("替换为")
        self.replace_btn = QPushButton("替换")
        self.replace_btn.clicked.connect(self.replace_text_current_segment)
        ...
        self.preview_status = QLabel("🔄 未生成")
        ...
        layout.addLayout(nav_layout)
        layout.addWidget(replace_frame)
    ...
    def on_preset_changed(self, index):
        ...
        if preset_info:
            ...
            self.current_preset_settings = settings
    ...
    def update_text_display(self):
        ...
        if self.search_mode:
             if self.search_results:
                 self.current_segment_index = self.search_results[self.search_index]
             else:
                 self.search_mode = False # No results, exit search mode

        self.text_edit.setText(self.processed_segments[self.current_segment_index])
        
        if self.search_mode:
            self.page_label.setText(f"搜索结果: {self.search_index + 1}/{len(self.search_results)} (段落 {self.current_segment_index + 1})")
            self.prev_btn.setEnabled(self.search_index > 0)
            self.next_btn.setEnabled(self.search_index < len(self.search_results) - 1)
    ...
    def replace_text_current_segment(self):
    ...
    def generate_preview(self):
        """生成试听音频"""
        preset_name = self.preset_combo.currentText()
        if not preset_name or preset_name == "新建预设...":
            QMessageBox.warning(self, "提示", "请先选择一个有效的预设")
            return
        
        self.save_text() # Save current text before generating
        ...
        try:
            ...
            self.gpt_sovits.load_models(gpt_path, self.current_preset_settings['sovits_path'])
            self.gpt_sovits.set_preset(self.current_preset_settings)
            ...
            if preview_path:
                ...
                self.reset_preview_state(error=True)
                QMessageBox.warning(self, "错误", "生成试听音频失败")
        except Exception as e:
            self.reset_preview_state(error=True)
            QMessageBox.critical(self, "错误", f"生成预览时发生未知错误: {e}")

    def reset_preview_state(self, error=False):
        """重置预览状态"""
        self.preview_btn.setEnabled(True)
        self.preview_status.setText("❌ 生成失败" if error else "🔄 未生成")
        self.play_btn.setEnabled(self.preview_generated and not error)
        self.preview_progress.setEnabled(self.preview_generated and not error)

    def on_preview_generated(self):
        """预览音频生成完成后的处理"""
        self.preview_generated = True
        self.reset_preview_state(error=False)
    ...
    def play_preview(self):
        ...
        try:
            if self.player.playbackState() == QMediaPlayer.PlaybackState.PlayingState:
                ...
                if self.player.source().url() != QUrl.fromLocalFile(self.preview_path).url() or self.player.mediaStatus() == QMediaPlayer.MediaStatus.EndOfMedia:
    ...
    def get_preview_text(self, text: str, limit: int = 100) -> str:
        ...
        if len(text) <= limit:
            return text
        end_marks = ["。", "！", "？", ".", "!", "?"]
        pos = text.rfind(tuple(end_marks), 0, limit)
        return text[:pos+1] if pos != -1 else text[:limit]
    ...
    def search_text(self):
        ...
        self.search_results = [i for i, seg in enumerate(self.processed_segments) if search_query.lower() in seg.lower()]
    ...
    def highlight_search_text(self):
        ...
        start = 0
        while (start := text.lower().find(self.last_search_query.lower(), start)) != -1:
            cursor.setPosition(start)
            cursor.movePosition(QTextCursor.MoveOperation.Right, QTextCursor.MoveMode.KeepAnchor, len(self.last_search_query))
            cursor.mergeCharFormat(highlight_format)
            start += len(self.last_search_query)
                                    ...
                                    for j in range(w.layout().count()):
                                        ...
                                        if isinstance(item, QWidget) and item.layout() and isinstance(item.layout(), QHBoxLayout):
                                            ...
                                            for k in range(control_layout.count()):
                                                ...
                                                if self._is_selector_widget(control_item):
...
class SegmentProcessCard(Card):
    # 请将这 3 个新方法，完整地复制并粘贴到 class SegmentProcessCard(Card): 的内部
    ...
    def __init__(self, segments: List[str], characters: List[Tuple[str, str]], preset_manager: PresetManager, preset_order_manager: PresetOrderManager = None, parent=None):
        ...
        self.preset_order_manager = preset_order_manager
        self.current_segment_index = 0
        self.global_narration_preset = "🔮卡芙卡"
        self.global_narration_emotion = "平静"
        
        # --- 新增：在这里添加记忆阈值的功能 ---
        self.narration_delete_threshold = 10  # 默认值为10
        # --- 新增结束 ---
    ...
    def on_narration_threshold_changed(self, value):
        """当旁白删除阈值改变时，更新实例变量以实现记忆功能"""
        self.narration_delete_threshold = value

    def delete_short_narrations(self):
        """一键删除当前段落中所有小于指定字数的旁白文本框内容"""
        threshold = self.narration_delete_threshold
        
        reply = QMessageBox.question(
            self,
            "确认操作",
            f"确定要清空当前段落中所有字数小于 {threshold} 的旁白内容吗？\n\n此操作会清空对应文本框，但不会删除文本框本身。",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No
        )
        if reply == QMessageBox.StandardButton.No:
        ...
        deleted_count = 0
        # 从第1个widget开始遍历，跳过第0个widget（全局替换与功能框）
            ...
            if isinstance(widget, QFrame):
                ...
                if is_narration:
                    ...
                    if text_edit:
                        ...
                        # 检查字数是否大于0且小于阈值
                        if 0 < len(text_content) < threshold:
                            text_edit.clear() # 清空文本框
                            deleted_count += 1
        
        QMessageBox.information(self, "操作完成", f"已成功清空 {deleted_count} 个短旁白文本框的内容。")

# 请将这个新方法，完整地复制并粘贴到 class SegmentProcessCard(Card): 的内部

    def delete_all_narrations(self):
        """一键删除当前段落中所有的旁白文本框内容"""
        reply = QMessageBox.question(
            self,
            "确认操作",
            f"危险操作：确定要清空当前段落中所有的旁白内容吗？\n\n此操作不可撤销，会清空所有旁白，无论字数多少。",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No
        )
        if reply == QMessageBox.StandardButton.No:
        ...
        deleted_count = 0
        # 从第1个widget开始遍历，跳过第0个widget（全局功能框）
        for i in range(1, self.content_layout.count()):
            widget = self.content_layout.itemAt(i).widget()
            if isinstance(widget, QFrame):
                # 通过背景颜色判断这个框是否是旁白
        ...
        QMessageBox.information(self, "操作完成", f"已成功清空 {deleted_count} 个旁白文本框的内容。")

    def _is_selector_widget(self, widget):
        """检查widget是否是选择器组件（QComboBox）"""
...
# 请用这个完整、未经省略的版本，替换掉 class SegmentProcessCard(Card): 中现有的 process_current_segment 方法

    # 请用这个完整的新版本，替换掉 class SegmentProcessCard(Card): 中现有的 process_current_segment 方法

    def process_current_segment(self):
        """处理当前段落，创建UI组件 (已添加“删除所有旁白”功能)"""
        # 清空现有内容
        ...
        narration_preset_container = QWidget()
        narration_preset_layout = QVBoxLayout(narration_preset_container)
        narration_preset_layout.setContentsMargins(0, 0, 0, 0)
        narration_preset_layout.setSpacing(3)
        narration_combo = QComboBox()
        all_presets = [preset[0] for preset in self.preset_manager.get_all_presets()]
        if self.preset_order_manager:
            sorted_presets = self.preset_order_manager.sort_presets(all_presets)
        ...
        narration_preset_layout.addWidget(narration_combo)
        narration_quick_layout = QHBoxLayout()
        narration_quick_layout.setSpacing(5)
        narration_first_btn = QPushButton("主角1")
        narration_first_btn.setFixedHeight(20)
        narration_first_btn.setStyleSheet("""
            QPushButton {
                background-color: #e8f4f8; border: 1px solid #c0d8e0; border-radius: 4px;
                padding: 2px 8px; color: #4a5568; font-size: 9px;
            }
            QPushButton:hover { background-color: #d0e8f0; border-color: #a8c8d0; }
            QPushButton:pressed { background-color: #b8d8e8; }
        """)
        narration_second_btn = QPushButton("主角2")
        narration_second_btn.setFixedHeight(20)
        narration_second_btn.setStyleSheet("""
            QPushButton {
                background-color: #f8e8f0; border: 1px solid #e0c0d0; border-radius: 4px;
                padding: 2px 8px; color: #4a5568; font-size: 9px;
            }
            QPushButton:hover { background-color: #f0d8e8; border-color: #d0a8c0; }
            QPushButton:pressed { background-color: #e8c8d8; }
        """)
        narration_first_btn.clicked.connect(lambda checked, nc=narration_combo: self.set_preset_to_lead(nc, True))
        narration_second_btn.clicked.connect(lambda checked, nc=narration_combo: self.set_preset_to_lead(nc, False))
        narration_quick_layout.addWidget(narration_first_btn)
        narration_quick_layout.addWidget(narration_second_btn)
        narration_quick_layout.addStretch()
        narration_preset_layout.addLayout(narration_quick_layout)
        narration_emotion_label = QLabel("旁白情绪:")
        narration_emotion_combo = QComboBox()
        if all_presets:
            emotions = self.preset_manager.get_preset_emotions(all_presets[0])
            narration_emotion_combo.addItems(emotions)
        disable_combo_wheel_scrolling(narration_emotion_combo)
        if hasattr(self, 'global_narration_preset') and self.global_narration_preset in all_presets:
            ...
            if index >= 0:
                ...
                if hasattr(self, 'global_narration_emotion') and self.global_narration_emotion in emotions:
                    ...
                    if emo_index >= 0: narration_emotion_combo.setCurrentIndex(emo_index)
        narration_apply = QPushButton("应用旁白预设")
        narration_apply.setStyleSheet("background-color: #C67A8A; color: white; border: none;")
        narration_layout.addWidget(narration_label)
        narration_layout.addWidget(narration_preset_container)
        ...
        delete_narration_layout = QHBoxLayout()
        delete_narration_label = QLabel("删除小于 N 字的旁白:")
        self.narration_delete_spinbox = QSpinBox()
        self.narration_delete_spinbox.setRange(1, 999)
        self.narration_delete_spinbox.setValue(self.narration_delete_threshold)
        delete_narration_button = QPushButton("执行删除")
        delete_narration_button.setStyleSheet("""
            QPushButton {
                background-color: #C5D8EC; color: #4a5568; border: 1px solid #d4c5b9;
            }
            QPushButton:hover { background-color: #B5C8DC; }
        """)
        
        # --- 新增：在这里添加“删除所有旁白”按钮 ---
        delete_all_narrations_button = QPushButton("删除所有旁白")
        delete_all_narrations_button.setStyleSheet("""
            QPushButton {
        ...
        delete_narration_layout.addWidget(delete_narration_label)
        delete_narration_layout.addWidget(self.narration_delete_spinbox)
        delete_narration_layout.addWidget(delete_narration_button)
        delete_narration_layout.addWidget(delete_all_narrations_button) # 添加新按钮到布局
        delete_narration_layout.addStretch()
        replace_layout.addLayout(delete_narration_layout)
        ...
        remove_history_button.clicked.connect(lambda: self.remove_selected_history(history_combo))
        apply_all_history_button.clicked.connect(self.apply_all_history)
        self.narration_delete_spinbox.valueChanged.connect(self.on_narration_threshold_changed)
        delete_narration_button.clicked.connect(self.delete_short_narrations)
        
        # --- 新增：连接新功能的信号 ---
        delete_all_narrations_button.clicked.connect(self.delete_all_narrations)
        # --- 新增连接结束 ---
        ...
        for segment_id, (segment_type, content, speaker) in enumerate(segments):
            ...
            segment_frame.setStyleSheet(f"QFrame {{ border: 1px solid #d4c5b9; border-radius: 8px; background-color: {('#faf6f1' if segment_type == 'narration' else '#ffffff')}; padding: 10px; margin: 5px; }}")
            ...
            if segment_type == "dialogue" and speaker:
                ...
                speaker_label.setStyleSheet("color: #C67A8A; font-weight: bold; margin-bottom: 4px;")
            ...
            text_edit.setStyleSheet(f"QTextEdit {{ border: 1px solid #e0e0e0; border-radius: 4px; padding: 8px; background-color: {('#faf6f1' if segment_type == 'narration' else '#ffffff')}; }}")
            ...
            sequence_label.setStyleSheet("QLabel { color: #C67A8A; font-weight: bold; font-size: 12px; margin: 2px; }")
            ...
            copy_btn.setStyleSheet("QPushButton { background-color: #C67A8A; color: white; border: none; padding: 5px; max-width: 60px; border-radius: 4px; } QPushButton:hover { background-color: #B56A7A; }")
            copy_btn.clicked.connect(lambda _, te=text_edit: self.copy_text_content(te))
            
            cut_btn = QPushButton("剪切")
            cut_btn.setStyleSheet("QPushButton { background-color: #C67A8A; color: white; border: none; padding: 5px; max-width: 60px; border-radius: 4px; } QPushButton:hover { background-color: #B56A7A; }")
            cut_btn.clicked.connect(lambda _, te=text_edit: self.cut_text_content(te))
            
            delete_btn = QPushButton("删除")
            delete_btn.setStyleSheet("QPushButton { background-color: #C67A8A; color: white; border: none; padding: 5px; max-width: 60px; border-radius: 4px; } QPushButton:hover { background-color: #B56A7A; }")
            delete_btn.clicked.connect(lambda _, te=text_edit: self.delete_text_content(te))
            
            save_btn = QPushButton("保存")
            save_btn.setStyleSheet("QPushButton { background-color: #C5D8EC; color: white; border: none; padding: 5px; max-width: 60px; border-radius: 4px; } QPushButton:hover { background-color: #B5C8DC; }")
            save_btn.clicked.connect(lambda _, te=text_edit: self.save_text_content(te))
            
            paste_btn = QPushButton("粘贴")
            paste_btn.setStyleSheet("QPushButton { background-color: #C5D8EC; color: white; border: none; padding: 5px; max-width: 60px; border-radius: 4px; } QPushButton:hover { background-color: #B5C8DC; }")
            ...
            preset_layout.addWidget(voice_combo)
            
            quick_select_layout = QHBoxLayout()
            quick_select_layout.setSpacing(5)
            first_lead_btn = QPushButton("主角1")
            first_lead_btn.setFixedHeight(20)
            first_lead_btn.setStyleSheet("""
                QPushButton {
                    background-color: #e8f4f8; border: 1px solid #c0d8e0; border-radius: 4px;
                    padding: 2px 8px; color: #4a5568; font-size: 9px;
                }
                ...
                QPushButton:pressed { background-color: #b8d8e8; }
            """)
            second_lead_btn = QPushButton("主角2")
            second_lead_btn.setFixedHeight(20)
            second_lead_btn.setStyleSheet("""
                QPushButton {
                    background-color: #f8e8f0; border: 1px solid #e0c0d0; border-radius: 4px;
                    padding: 2px 8px; color: #4a5568; font-size: 9px;
                }
                ...
                QPushButton:pressed { background-color: #e8c8d8; }
            """)
            first_lead_btn.clicked.connect(lambda checked, vc=voice_combo: self.set_preset_to_lead(vc, True))
            second_lead_btn.clicked.connect(lambda checked, vc=voice_combo: self.set_preset_to_lead(vc, False))
            quick_select_layout.addWidget(first_lead_btn)
            quick_select_layout.addWidget(second_lead_btn)
            quick_select_layout.addStretch()
            preset_layout.addLayout(quick_select_layout)
            ...
            if segment_type == "dialogue" and speaker in self.characters:
                ...
                if index >= 0: voice_combo.setCurrentIndex(index)
                emotions = self.preset_manager.get_preset_emotions(preset)
                emotion_combo.addItems(emotions)
                if emotions: emotion_combo.setCurrentIndex(0)
                disable_combo_wheel_scrolling(emotion_combo)
            else:
                if global_narration_preset:
                    ...
                    if global_narration_emotion and global_narration_emotion in emotions:
                        ...
                        if emotion_index >= 0: emotion_combo.setCurrentIndex(emotion_index)
                    elif emotions: emotion_combo.setCurrentIndex(0)
                else:
                    if all_presets:
            ...
            status_label.setStyleSheet("QLabel { font-size: 18px; min-width: 30px; }")
            
            voice_combo.currentTextChanged.connect(lambda text, ec=emotion_combo: self.on_voice_changed(text, ec))
            generate_btn.clicked.connect(lambda _, te=text_edit, vc=voice_combo, ec=emotion_combo, gb=generate_btn, rb=regenerate_btn, pb=preview_btn, sl=status_label: self.generate_audio(te, vc.currentText(), ec.currentText(), gb, rb, pb, sl, is_regenerate=False))
            regenerate_btn.clicked.connect(lambda _, te=text_edit, vc=voice_combo, ec=emotion_combo, gb=generate_btn, rb=regenerate_btn, pb=preview_btn, sl=status_label: self.generate_audio(te, vc.currentText(), ec.currentText(), gb, rb, pb, sl, is_regenerate=True))
            preview_btn.clicked.connect(lambda _, te=text_edit: self.preview_audio(te))
            
            control_layout.addWidget(QLabel("配音:"))
            control_layout.addWidget(preset_container)
    ...
    def set_preset_to_lead(self, combo_box: QComboBox, is_first: bool):
        """设置预设为主角1（第一个）或主角2（最后一个）"""
        if combo_box.count() == 0:
            return
            
        current_index = combo_box.currentIndex()
        target_index = 0 if is_first else combo_box.count() - 1
        
        # 只有当目标索引不同时才设置，避免不必要的信号触发
        if current_index != target_index:
            combo_box.setCurrentIndex(target_index)
            print(f"设置预设: {'主角1' if is_first else '主角2'}, 索引: {target_index}, 预设: {combo_box.currentText()}")
    ...
    def generate_audio(self, text_edit: QTextEdit, preset_name: str, emotion: str,
        ...
        print(f"[LOG] generate_audio called, preset={preset_name}, emotion={emotion}")
    ...
    def preview_audio(self, text_edit: QTextEdit):
        ...
        try:
            ...
            # --- 步骤 2: 使用更健壮的方式播放音频 ---
            if audio_path:
                # 无论如何，先停止当前播放，以实现“中断”效果
                self.player.stop()

                # 重新设置音频源，这能确保播放器为播放做好准备
                self.player.setSource(QUrl.fromLocalFile(audio_path))
                
                # 【关键修复】明确将播放位置移动到开头
                # 这解决了播放结束后无法再次播放同一文件的问题
                ...
                if 'logger' in globals():
                    logger.info(f"请求播放音频: {audio_path}")
            else:
                QMessageBox.warning(self, "提示", "音频文件不存在，请先生成或重新生成。")
        except Exception as e:
    ...
    # 请将这两个完整的方法，复制并粘贴到 SegmentProcessCard 类的内部

    # 请用这个完整版本替换现有的 generate_all_audio 方法
    # 请用这个带 print 的完整版本替换现有的 generate_all_audio 方法
    def generate_all_audio(self):
        """一键生成所有音频（V4.0 带 print 诊断）"""
        
        print("\n--- [诊断] generate_all_audio: 开始收集任务 ---")
        tasks_by_composite_key = {}
        text_box_widgets = {}
        
        for i in range(self.content_layout.count()):
            widget = self.content_layout.itemAt(i).widget()
            if i > 0 and isinstance(widget, QFrame):
                text_edit = widget.findChild(QTextEdit)
                all_combos = widget.findChildren(QComboBox)
                if text_edit and text_edit.toPlainText().strip() and len(all_combos) > 1:
                    preset_name = all_combos[0].currentText()
                    emotion = all_combos[1].currentText()
                    
                    composite_key = (preset_name, emotion)
                    if composite_key not in tasks_by_composite_key:
                        tasks_by_composite_key[composite_key] = []
                    
                    task_id = id(text_edit)
                    tasks_by_composite_key[composite_key].append(task_id)
                    ...
                    for label in labels:
                        if label.text() in ["🔄", "⏳", "🌷", "🥳","❌", "❓"]:
                            status_label = label
                            break
                    generate_btn, regenerate_btn, preview_btn = None, None, None
                    for btn in all_buttons:
                        if btn.objectName() == "generate_btn": generate_btn = btn
                        elif btn.objectName() == "regenerate_btn": regenerate_btn = btn
                        elif btn.text() == "试听": preview_btn = btn
                    
                    text_box_widgets[task_id] = (text_edit, generate_btn, regenerate_btn, preview_btn, status_label)

        if not text_box_widgets:
            QMessageBox.information(self, "提示", "没有找到需要生成的文本")
            return

        self.sorted_tasks = []
        for (preset_name, emotion), task_ids in tasks_by_composite_key.items():
            for task_id in task_ids:
                self.sorted_tasks.append((preset_name, task_id, emotion))
        
        print(f"--- [诊断] generate_all_audio: 任务收集完成，总计 {len(self.sorted_tasks)} 个任务。")
        print("--- [诊断] generate_all_audio: 创建的最优任务清单如下 ---")
        for idx, (p, tid, e) in enumerate(self.sorted_tasks):
            print(f"  - 任务 #{idx+1}: 配音员='{p}', 情绪='{e}'")
        print("--------------------------------------------------\n")
        
        self.text_box_widgets = text_box_widgets
        self.progress_dialog = QProgressDialog("正在批量生成音频...", "取消", 0, len(self.sorted_tasks), self)
        self.progress_dialog.setWindowTitle("批量生成")
        self.progress_dialog.setWindowModality(Qt.WindowModality.WindowModal)
        self.progress_dialog.setMinimumWidth(350)
        self.progress_dialog.setPalette(QApplication.palette())
        self.progress_dialog.setStyleSheet("""
            QProgressDialog {
                background-color: #faf6f1; color: #4a5568; border: 1px solid #d4c5b9;
                border-radius: 8px; min-height: 120px;
            }
            QProgressDialog QLabel { color: #4a5568; background-color: transparent; }
            QProgressBar {
                background-color: #ffffff; border: 2px solid #d4c5b9; border-radius: 10px;
                text-align: center; color: #4a5568; font-size: 12px;
                min-height: 24px; max-height: 24px; padding: 0px;
            }
            QProgressBar::chunk { background-color: #C67A8A; border-radius: 8px; margin: 2px; }
            QProgressDialog QPushButton {
                background-color: #C5D8EC; border: 1px solid #d4c5b9; border-radius: 6px;
                padding: 6px 12px; color: #4a5568; min-width: 80px; margin-top: 10px;
            }
            QProgressDialog QPushButton:hover { background-color: #B5C8DC; }
        """)
        
        self.current_batch_index = 0
        self.last_loaded_preset = None
        self.last_emotion_used = None
        self.progress_dialog.show()
        self._process_batch_item()

    # 也请用这个带 print 的完整版本替换现有的 _process_batch_item 方法
    def _process_batch_item(self):
        """（V4.0 带 print 诊断）处理单个批量任务"""
        if self.progress_dialog.wasCanceled() or self.current_batch_index >= len(self.sorted_tasks):
            self.progress_dialog.close()
            if not self.progress_dialog.wasCanceled():
                 QMessageBox.information(self, "完成", "所有音频已生成完毕！")
            return

        preset_name, task_id, emotion = self.sorted_tasks[self.current_batch_index]
        text_edit, generate_btn, regenerate_btn, preview_btn, status_label = self.text_box_widgets[task_id]
        
        print(f"==> [诊断] 开始处理任务 #{self.current_batch_index + 1}")
        print(f"    - 任务要求: 配音员='{preset_name}', 情绪='{emotion}'")
        
        self.progress_dialog.setValue(self.current_batch_index)
        self.progress_dialog.setLabelText(f"正在处理第 {self.current_batch_index + 1}/{len(self.sorted_tasks)} 条音频...")
        QApplication.processEvents()
        ...
        try:
            ...
            last_preset = getattr(self, 'last_loaded_preset', None)
            last_emotion = getattr(self, 'last_emotion_used', None)
            print(f"    - 程序记忆: 上次配音员='{last_preset}', 上次情绪='{last_emotion}'")

            if preset_name != last_preset or emotion != last_emotion:
                print(f"    - 决策: 检测到状态变化，需要更新。")
                if preset_name != last_preset:
                    print(f"      - 原因: 配音员发生变化 ('{last_preset}' -> '{preset_name}')")
                if emotion != last_emotion:
                    print(f"      - 原因: 情绪发生变化 ('{last_emotion}' -> '{emotion}')")

                preset_info = self.preset_manager.get_preset(preset_name)
                if not preset_info: raise Exception(f"找不到预设 {preset_name}")
                settings, _ = preset_info

                if preset_name != last_preset:
                    print(f"    - 执行: 正在调用 load_models...")
                    gpt_path = settings.get('model_path', '') or settings.get('gpt_path', '')
                    sovits_path = settings.get('sovits_path', '')
                    if not self.gpt_sovits.load_models(gpt_path, sovits_path):
                        raise Exception(f"加载模型失败: {preset_name}")
                
                print(f"    - 执行: 正在调用 set_preset, 使用情绪='{emotion}'...")
                settings['ref_emotion'] = emotion
                if not self.gpt_sovits.set_preset(settings):
                    raise Exception(f"设置预设失败: {preset_name}")
                
                self.last_loaded_preset = preset_name
                self.last_emotion_used = emotion
                print(f"    - 更新记忆: 最新配音员='{self.last_loaded_preset}', 最新情绪='{self.last_emotion_used}'")
            else:
                print(f"    - 决策: 状态未变化，跳过所有准备工作。")
            ...
            audio_filename = f"{source_filename}_段落{segment_index+1:04d}_文本框{box_index+1:02d}.wav"
            audio_path = os.path.join(audio_dir, audio_filename)
            text_to_generate = text_edit.toPlainText().strip()
            success = self.gpt_sovits.generate_audio(text_to_generate, audio_path, emotion=emotion)

            if success:
                self.segment_audio_states[text_edit_id] = (True, audio_path)
                status_label.setText("🌷")
                if generate_btn: generate_btn.setEnabled(False)
                if regenerate_btn: regenerate_btn.setEnabled(True)
                if preview_btn: preview_btn.setEnabled(True)
            else:
                status_label.setText("❌")
        except Exception as e:
            status_label.setText("❌")

        print(f"<== [诊断] 任务 #{self.current_batch_index + 1} 处理结束\n")
        self.current_batch_index += 1
        QTimer.singleShot(0, self._process_batch_item)
    ...
    def apply_narration_preset(self, preset_name: str, emotion: str):
            ...
            # --- 核心修复：在这里更新“记忆”变量 ---
            # 1. 保存当前选择的全局预设和情绪，以便在切换段落时能被记住
            self.save_narration_preset(preset_name)
            self.save_narration_emotion(emotion)
            # --- 修复结束 ---
                    ...
                    if text_edit and hasattr(text_edit, "objectName"):
                        ...
                        if box_idx < len(segments) and (
                            ...
                            if control_panel:
                                ...
                                # 查找预设和情绪下拉框
                                # 修改查找逻辑，确保找到正确的下拉框
                                combos = control_panel.findChildren(QComboBox)
                                if len(combos) >= 2:
                                    preset_combo = combos[0]
                                    emotion_combo = combos[1]
                                ...
                                if preset_combo:
                                    ...
                                    if index >= 0:
                                        preset_combo.setCurrentIndex(index)
                                        # 手动触发情绪更新
                                        self.on_voice_changed(preset_name, emotion_combo)
